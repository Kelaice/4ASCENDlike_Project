...
while running:
    ...
    match state:
        ...
        case 2:  # PVP
            ...
            board.drawUI(screen, game.hp1, game.hp2, game.max_hp1,
                         game.max_hp2, delta_time)
            ...
            if battle_animation_started and not board.is_battle_animation_active(
                ...
                if pending_game_state is not None:
                    ...
                    # 创建用于显示的临时状态
                    display_state = (pieces, magic_plants, ascend_state,
                                     game.hp1, game.hp2, 0)
                    # 临时同步棋盘显示
                ...
                for event in events:
                    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                        if board.check_menu_button_click(
                                mouse_pos) and click_cooldown <= 0:
                            state = MENU
                            menu_animation.reset()
                            game = FourAscendGame()
                            board.set_game_reference(game)  # 重新设置游戏引用
                            game_state = game.getInitBoard()
                            board.syncFromPieces(game_state)  # 同步棋盘视觉显示
                            player = 1
                            click_cooldown = CLICK_COOLDOWN_TIME
                            # 重置动画状态
                            battle_animation_started = False
                            pending_game_state = None
                            pending_player = None
                            board.reset_battle_animation()  # 清空战斗动画
                            ...
                            if 0 <= action < len(
                                    valid_moves) and valid_moves[action]:
                                # 检查是否处于防御阶段
                ...
                if player == 1:  # 玩家回合
                    ...
                    for event in events:
                        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                            if board.check_menu_button_click(
                                ...
                                board.reset_battle_animation()  # 清空战斗动画
                                # 重新创建AI玩家
                                if 'ai_player' in globals():
                                    del globals()['ai_player']
                            elif valid and not lock and click_cooldown <= 0:
                                pixel_pos = board.findPos()
                                row, col = board.pixel_to_board_index(
                                    pixel_pos)
                                action = row * board.Boardsize + col
                                valid_moves = game.getValidMoves(
                                    game_state, player)
                    ...
                    if ai_thinking_time >= 800:
                        if 'ai_player' not in globals():
                            globals()['ai_player'] = StupidFourAscendPlayer(
                                game)
                        ...
                        thinking_text = font.render("Thinking...", True,
                                                    (100, 100, 100))
        ...
        case 4:  # TEACH - 教学模式
            ...
            if teaching_anim_preparing:
                ...
                if teaching_anim_timer > 500:
                    ...
                    if step == 2:
                        # 攻击动画
                        board.start_battle_animation(teaching_state, 4 * 9 + 4,
                                                     1, 2)
                    elif step == 3:
                        # 防御动画
                        board.start_battle_animation(teaching_state, 4 * 9 + 5,
                                                     -1, 1)
            elif teaching_anim_playing:
                teaching_anim_timer += delta_time
                # 检查动画是否完成
                if not board.is_battle_animation_active(
            ...
            if step < 6 and not teaching_anim_playing and not teaching_anim_preparing:
                ...
                hint_text = hint_font.render("点击屏幕继续...", True,
                                             (100, 100, 100))
            ...
            if step == 0:
                ...
                title = title_font.render("欢迎来到4ASCEND!", True, (0, 0, 0),
                                          (255, 255, 255))
                screen.blit(title, (400, 50))

                content_font = pygame.font.Font("resource\\pixelfont.ttf", 36)
                text1 = content_font.render("这是一款融合了四子棋与策略元素的对战游戏", True,
                                            (0, 0, 0), (255, 255, 255))
                text2 = content_font.render("你的目标是将对手的血量降至0", True, (0, 0, 0),
                                            (255, 255, 255))
                ...
                # 步骤1：基本规则 - 棋盘和落子
                title_font = pygame.font.Font("resource\\pixelfont.ttf", 48)
                title = title_font.render("游戏棋盘与落子", True, (0, 0, 0),
                                          (255, 255, 255))
                screen.blit(title, (430, 50))

                content_font = pygame.font.Font("resource\\pixelfont.ttf", 36)
                text1 = content_font.render("• 游戏在9x9的棋盘上进行", True, (0, 0, 0),
                                            (255, 255, 255))
                text2 = content_font.render("• 玩家轮流在空位上放置自己的棋子", True,
                                            (0, 0, 0), (255, 255, 255))
                text3 = content_font.render("• 黑方先行，白方后行", True, (0, 0, 0),
                                            (255, 255, 255))
                screen.blit(text1, (250, 150))
                ...
                teaching_state = (pieces, magic_plants, ascend_state, game.hp1,
                                  game.hp2, 0)
                ...
                # 步骤2：攻击阶段
                title_font = pygame.font.Font("resource\\pixelfont.ttf", 48)
                title = title_font.render("攻击阶段", True, (0, 0, 0),
                                          (255, 255, 255))
                screen.blit(title, (500, 50))

                content_font = pygame.font.Font("resource\\pixelfont.ttf", 36)
                text1 = content_font.render("• 当你放置棋子形成4个或更多连续棋子时", True,
                                            (0, 0, 0), (255, 255, 255))
                text2 = content_font.render("• 这些棋子会被消除并进入'ASCEND'状态", True,
                                            (0, 0, 0), (255, 255, 255))
                text3 = content_font.render("• ASCEND 状态的棋子会在下一轮发动攻击", True,
                                            (0, 0, 0), (255, 255, 255))
                text4 = content_font.render("• 多个方向的连子会同时触发", True, (0, 0, 0),
                                            (255, 255, 255))
                screen.blit(text1, (250, 150))
                screen.blit(text2, (250, 200))
                screen.blit(text3, (250, 250))
                ...
                # 触发攻击动画准备阶段（只触发一次）
                if step not in teaching_animation_triggered and not teaching_anim_playing and not teaching_anim_preparing:
                ...
                # 步骤3：防御阶段
                title_font = pygame.font.Font("resource\\pixelfont.ttf", 48)
                title = title_font.render("防御阶段", True, (0, 0, 0),
                                          (255, 255, 255))
                screen.blit(title, (500, 50))

                content_font = pygame.font.Font("resource\\pixelfont.ttf", 36)
                text1 = content_font.render("• 当一方发动攻击后，另一方就会进入防御阶段", True,
                                            (0, 0, 0), (255, 255, 255))
                text2 = content_font.render("• 防御方同样可以触发四连，获得防御力", True,
                                            (0, 0, 0), (255, 255, 255))
                text3 = content_font.render("• 防御力与防御方连子数量成正比", True,
                                            (0, 0, 0), (255, 255, 255))
                text4 = content_font.render("• 特殊规则：", True, (0, 0, 0),
                                            (255, 255, 255))
                text5 = content_font.render("• 防御方可以在上一轮消除的棋子处放置自己的棋子", True,
                                            (0, 0, 0), (255, 255, 255))
                text6 = content_font.render("• 此时这个位置的攻击将会无效化", True,
                                            (0, 0, 0), (255, 255, 255))
                screen.blit(text1, (250, 150))
                screen.blit(text2, (250, 200))
                screen.blit(text3, (250, 250))
                ...
                # 步骤4：魔法植物系统
                title_font = pygame.font.Font("resource\\pixelfont.ttf", 48)
                title = title_font.render("魔法植物", True, (0, 0, 0),
                                          (255, 255, 255))
                screen.blit(title, (500, 50))

                content_font = pygame.font.Font("resource\\pixelfont.ttf", 36)
                text1 = content_font.render("• 棋盘上会随机生成黄色的魔法植物", True,
                                            (0, 0, 0), (255, 255, 255))
                text2 = content_font.render("• 魔法植物可以增强棋子的攻击力或防御力", True,
                                            (0, 0, 0), (255, 255, 255))
                text3 = content_font.render("• 每回合可能生成新的魔法植物", True, (0, 0, 0),
                                            (255, 255, 255))
                text4 = content_font.render("• 占领魔法植物的棋子会获得特殊标记", True,
                                            (0, 0, 0), (255, 255, 255))
                screen.blit(text1, (250, 150))
                screen.blit(text2, (250, 200))
                screen.blit(text3, (250, 250))
                ...
                # 步骤5：血量与胜利条件
                title_font = pygame.font.Font("resource\\pixelfont.ttf", 48)
                title = title_font.render("血量与胜利", True, (0, 0, 0),
                                          (255, 255, 255))
                screen.blit(title, (480, 50))

                content_font = pygame.font.Font("resource\\pixelfont.ttf", 36)
                text1 = content_font.render("• 双方初始有6点血量", True, (0, 0, 0),
                                            (255, 255, 255))
                text2 = content_font.render("• 当攻击超过防御时，对手会受到伤害", True,
                                            (0, 0, 0), (255, 255, 255))
                text3 = content_font.render("• 伤害值 = 攻击力 - 防御力", True,
                                            (0, 0, 0), (255, 255, 255))
                text4 = content_font.render("• 将对手血量降至0即可获胜！", True, (0, 0, 0),
                                            (255, 255, 255))
                screen.blit(text1, (250, 150))
                screen.blit(text2, (250, 200))
                screen.blit(text3, (250, 250))
                ...
                # 步骤6：结束教学
                title_font = pygame.font.Font("resource\\pixelfont.ttf", 48)
                title = title_font.render("教学完成！", True, (0, 0, 0),
                                          (255, 255, 255))
                screen.blit(title, (500, 50))

                content_font = pygame.font.Font("resource\\pixelfont.ttf", 36)
                text1 = content_font.render("现在你已经了解了4ASCEND的基本规则", True,
                                            (0, 0, 0), (255, 255, 255))
                text2 = content_font.render("点击MENU返回主菜单开始游戏吧！", True,
                                            (0, 0, 0), (255, 255, 255))
                screen.blit(text1, (250, 150))
                screen.blit(text2, (250, 200))
            ...
            for event in events:
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    if board.check_menu_button_click(mouse_pos):
                        # 返回菜单，清理教学相关变量
                        if 'teaching_step' in globals():
                            del globals()['teaching_step']
                        if 'teaching_game' in globals():
                            del globals()['teaching_game']
                        if 'teaching_board' in globals():
                            # 重置棋盘和动画
                            teaching_board.reset_battle_animation()
                            # 添加棋盘重置：创建空棋盘状态并同步
                            empty_pieces = np.zeros((9, 9), dtype=np.int8)
                            empty_magic_plants = np.zeros((9, 9),
                                                          dtype=np.int8)
                            empty_ascend_state = np.zeros((9, 9),
                                                          dtype=np.bool_)
                            empty_state = (empty_pieces, empty_magic_plants,
                                           empty_ascend_state, 6, 6, 0)
                            teaching_board.syncFromPieces(empty_state)
                            del globals()['teaching_board']
                        if 'teaching_anim_playing' in globals():
                            del globals()['teaching_anim_playing']
                        if 'teaching_anim_timer' in globals():
                            del globals()['teaching_anim_timer']
                        if 'teaching_animation_triggered' in globals():
                            del globals()['teaching_animation_triggered']
                        if 'teaching_anim_preparing' in globals():
                            del globals()['teaching_anim_preparing']
                        state = MENU
                        menu_animation.reset()
                    else:
                        # 切换到下一步教学
