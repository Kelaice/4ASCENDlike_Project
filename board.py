...
class Board:
    ...
    def __init__(self):
        ...
        self.RightDown_Point = (871 - self.Board_x_move,
                                621 - self.Board_y_move)
    ...
    def start_battle_animation(self, game_state, defending_action,
                               attacking_player, defending_player):
        self.battle_animation_active = True
        self.battle_animation.start(game_state, defending_action,
                                    attacking_player, defending_player)
    ...
    def draw_place_animations(self, screen):
        for anim in self.place_animations:
            ...
            pos_x = self.LeftUP_Point[
                0] + anim['col'] * self.Distance - new_size[0] // 2
            pos_y = self.LeftUP_Point[
                1] + anim['row'] * self.Distance - new_size[1] // 2

            screen.blit(temp_surface, (pos_x, pos_y))

    def draw_menu_button(self, screen, mouse_pos):
        self.menu_button_hovered = self.menu_button_rect.collidepoint(
            mouse_pos)
    ...
    def update_hurt_animations(self, delta_time, current_hp1, current_hp2):
        ...
        # 更新动画状态
        for animation in [
                self.player1_hurt_animation, self.player2_hurt_animation
        ]:
            if animation['active']:
        ...
        for x in range(original_image.get_width()):
            for y in range(original_image.get_height()):
                ...
                if original_color[3] > 0:  # 如果不是完全透明
                    ...
                    red_alpha = min(
                        255, int(alpha_value * (original_color[3] / 255.0)))
    ...
    def drawBoard(self, screen):
        #绘制棋盘
        origin = pygame.Vector2(screen.get_width() / 2,
                                screen.get_height() / 2)
        xfirstLine_start = origin - pygame.Vector2(self.XtoXstart,
                                                   self.YtoXstart)  #横向绘制起点
        yfirstLine_start = origin - pygame.Vector2(self.XtoYstart,
                                                   self.YtoYstart)  #纵向绘制起点
    ...
    def drawState(self, screen, hp1=6, hp2=6, initial_hp1=6, initial_hp2=6):
        ...
        # 应用抖动偏移绘制头像
        player1_pos = (self.PlayerHead_Pos[0] + player1_shake_x,
                       self.PlayerHead_Pos[1] + player1_shake_y)
        player2_pos = (self.EnemyHead_Pos[0] + player2_shake_x,
                       self.EnemyHead_Pos[1] + player2_shake_y)

        screen.blit(player_image, player1_pos)
        screen.blit(enemy_image, player2_pos)

        if self.player1_hurt_animation[
                'active'] and self.player1_hurt_animation['red_alpha'] > 0:
        ...
        if self.player2_hurt_animation[
                'active'] and self.player2_hurt_animation['red_alpha'] > 0:
            hurt_overlay = self.create_hurt_overlay(
    ...
    def findPos(self):
        ...
        for i in range(self.LeftUP_Point[0], self.RightDown_Point[0] + 5,
                       self.Distance):
            for j in range(self.LeftUP_Point[1], self.RightDown_Point[1] + 5,
                           self.Distance):
                L = i - self.CheckRadius
                R = i + self.CheckRadius
                U = j + self.CheckRadius
                D = j - self.CheckRadius
                if (mousePos[0] > L and mousePos[0] < R and mousePos[1] > D
                        and mousePos[1] < U):
    ...
    def checkMouse(self, screen, player):
        ...
        if piecePos[0] != 0 and self.checkPiece(piecePos):
            if player == 1:
                pygame.draw.circle(screen, 0xE4080A,
                                   [piecePos[0], piecePos[1]], 20, 2)
            else:
                pygame.draw.circle(screen, 0x5697FF,
                                   [piecePos[0], piecePos[1]], 20, 2)
            return True
        return False
        ...
        for row in range(self.Boardsize):
            for col in range(self.Boardsize):
                if self.ascend_state[row][col]:
                    ...
                    scale_factor = 0.9 + 0.1 * abs(
                        math.sin(self.animation_time / 24))
                    ...
                    scaled_image = pygame.transform.scale(
                        chess_image, new_size)

                    temp_surface = pygame.Surface(new_size, pygame.SRCALPHA)
                    temp_surface.blit(scaled_image, (0, 0))
                    temp_surface.set_alpha(
                        int(128 - scale_factor * 10))  # 设置透明度为0.5 (128/255)

                    pos_x = self.LeftUP_Point[
                        0] + col * self.Distance - new_size[0] // 2
                    pos_y = self.LeftUP_Point[
